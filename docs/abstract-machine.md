# StarrySkyL4 AbstractMachine 环境使用说明
...
`third_party/am-kernels`，不需要安装、选择或配置外部路径。该环境不能与
`-isolated` 混用。

工程的 `configs/abstract-machine.mk` 记录以下路径：

- `ECOS_SDK_HOME`：当前 embedded-sdk 根目录。
- `AM_HOME`：SDK 内迁入的 `third_party/abstract-machine`。

程序源码路径由工程 Makefile 固定为 `$(ECOS_SDK_HOME)/third_party/am-kernels`，不接受
工程配置、环境变量或命令行覆盖。

SDK 内的 AbstractMachine 是面向 StarrySkyL4 的精简副本，只保留
`riscv32e-ysyxsoc` 架构规则、AM/Klib 公共接口和 RV32E 所需的软算术
实现；其他 ISA、仿真器平台和旧设备实现不随 SDK 分发。

SDK 移动后，可以修改 `ECOS_SDK_HOME` 或重新创建工程。执行
`ecos set_board l4` 只刷新 AbstractMachine 环境元数据，不会覆盖工程根 Makefile。
...
`environments/abstract-machine/programs/manifest.mk` 中更新状态、所需 core/device features、验证等级和
