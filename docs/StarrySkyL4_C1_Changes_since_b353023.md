# StarrySkyL4_C1 完整变更日志

StarrySkyL4_C1 的 SDK AbstractMachine 环境见
[AbstractMachine 环境使用说明](abstract-machine.md)。

更新时间：2026-07-12

基准提交：`b353023719cac6d8dfe390158eba8f89067d6c4a`（`feat(board): add StarrySkyL4_C1 BSP support`）

本文档覆盖此前同名日志，记录 StarrySkyL4_C1 从基准提交到当前版本的完整有效变更。当前实现以 `verif/` 的启动、链接和 QSPI 行为为基准，并以 `l4test/st7735_clock` 的板上运行结果作为最终验证。
...
- I2C：支持 I2C0 的 7 位地址探测、连续字节写读，以及写后重复 START 读；SCL 为 GPIO0[27]、SDA 为 GPIO0[28]，分频参数范围为 1..256。
...
- 删除未在 L4 上验证的 PS2、RCU、RTC 和 WDG 板级驱动及对应 L4 模板；I2C 已迁移到
  ysyx-2512-1 的 3.0 HAL/Driver，提供完整字节事务和重复 START 读。
...
- StarrySkyL4_C1 BSP 声明 AbstractMachine 默认 core 为 `rv32e-base`；AM 工程不再要求
  或允许用户选择核心。
...
- `set_board l4c1` 会复制 board 目录中的 Makefile、链接脚本、启动文件和 bootloader。
...
- 全部 L4 C 模板的 `start.S` 和 `sections.lds` 与 `board/StarrySkyL4_C1` 单一来源保持一致。
...
- QSPI 新增 repeat API 当前由 StarrySkyL4_C1 驱动实现；跨板使用前需要对应 BSP 提供实现。
- 修改 board 目录的启动或链接文件后，应同步全部 `templates/*/l4c1`，并重新验证普通与 isolated 项目生成流程。
