...
static int gpio_map_hal_result(int result)
{
    if (result >= 0)
        return result;
    if (result == HAL_GPIO_ERROR_INVALID_ARGUMENT)
        return ECOS_GPIO_ERROR_INVALID_ARGUMENT;
    if (result == HAL_GPIO_ERROR_UNSUPPORTED)
        return ECOS_GPIO_ERROR_UNSUPPORTED;
    return ECOS_GPIO_ERROR_IO;
}
