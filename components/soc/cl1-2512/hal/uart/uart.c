                         ...
                         const hal_uart_config_t *config)
{
    ...
    REG_UART_0_LCR = UART_LCR_DLAB;
    REG_UART_0_DLL = UART0_DIVISOR;
    REG_UART_0_DLH = 0u;
    REG_UART_0_LCR = UART_LCR_8N1;
    REG_UART_0_MCR = 0u;
    return ECOS_OK;
}
...
int hal_uart_write(hal_uart_port_t port, const uint8_t *data, size_t size)
{
    ...
    for (index = 0u; index < size; ++index) {
        while ((REG_UART_0_LSR & UART_LSR_THR_EMPTY) == 0u)
            ;
        REG_UART_0_THR = data[index];
    }
    return (int)size;
}
...
int hal_uart_read(hal_uart_port_t port, uint8_t *data, size_t size)
{
    ...
    for (index = 0u; index < size; ++index) {
        while ((REG_UART_0_LSR & UART_LSR_DATA_READY) == 0u)
            ;
        data[index] = REG_UART_0_RBR;
    }
    return (int)size;
}
...
int hal_uart_try_read(hal_uart_port_t port, uint8_t *data)
{
    if (!uart_port_is_valid(port) || data == NULL)
        return ECOS_ERR_INVALID_ARGUMENT;
    if ((REG_UART_0_LSR & UART_LSR_DATA_READY) == 0u)
        return 0;
