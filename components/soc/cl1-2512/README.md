...
`cl1-2512` is the SDK 3.0 Target used by StartySky T1. It owns the
...
`0x01800000`. The legacy T1 SDRAM boot profile remains in the 2.x compatibility
BSP and is not selected by this Target.

Create and build a console example with:

```bash
python3 tools/ecos.py --sdk . project create hello --board t1
cd hello
ecos build
```
