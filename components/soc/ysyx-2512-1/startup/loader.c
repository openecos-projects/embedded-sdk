...
#define REG_GPIO_PADDIR(port) \
    (*((volatile uint32_t *)(0x10100000u + (uint32_t)(port) * 0x1000u)))
...
{
    /* 所有 GPIO 先置为输入安全态；QSPI/UART 等功能复用引脚不受 PADDIR 影响 */
    REG_GPIO_PADDIR(0) = 0u;
    REG_GPIO_PADDIR(1) = 0u;
    REG_GPIO_PADDIR(2) = 0u;
