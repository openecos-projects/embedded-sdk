...
#ifdef GPIO_GROUP_2
        case 2:
            REG_GPIO_2_PADDIR = REG_GPIO_2_PADDIR & ~(0x01 << io_offset);
            break;
#endif
        default:
            break;
...
void gpio_hal_set_level(uint8_t gpio_id, uint8_t gpio_num, uint8_t level){
    ...
    switch (io_group){
        ...
        case 2:
            if (level == GPIO_LEVEL_HIGH){
                REG_GPIO_2_PADOUT |= ((uint32_t)0x01 << io_offset);
            }
            else{
                REG_GPIO_2_PADOUT &= ~((uint32_t)0x01 << io_offset);
            }
...
#ifdef GPIO_GROUP_2
        case 2:
            return (REG_GPIO_2_PADIN & (1 << io_offset)) ? 1 : 0;
#endif
        default:
            return 0;
    }
...
void gpio_hal_set_fcfg(uint8_t gpio_id, uint8_t gpio_num, uint8_t val){
    ...
    switch (io_group){
        ...
        case 2:
            if(val == 1){
                REG_GPIO_2_IOFCFG = REG_GPIO_2_IOFCFG | (1 << io_offset);
            }else{
                REG_GPIO_2_IOFCFG = REG_GPIO_2_IOFCFG & ~(1 << io_offset);
            }
            break;
...
void gpio_hal_set_mux(uint8_t gpio_id, uint8_t gpio_num, uint8_t val){
    ...
    switch (io_group){
        ...
        case 2:
            if(val == 1){
                REG_GPIO_2_PINMUX = REG_GPIO_2_PINMUX | (1 << io_offset);
            }else{
                REG_GPIO_2_PINMUX = REG_GPIO_2_PINMUX & ~(1 << io_offset);
            }
            break;
