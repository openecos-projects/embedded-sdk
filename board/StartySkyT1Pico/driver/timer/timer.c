...
uint8_t startysky_t1_pico_timer_interrupt_active;

/**
 * 停止 Timer0 并清除已经锁存的完成状态。
 */
static void startysky_t1_pico_timer_stop_and_clear(void)
{
    uint32_t eoi_value;
 ...
 */
static uint8_t startysky_t1_pico_timer_wait_ticks(uint32_t ticks)
{
    uint32_t poll_count = 0u;
    uint32_t eoi_value;

    /* 配置前停止 Timer0 并清除历史完成状态。 */
    startysky_t1_pico_timer_stop_and_clear();

    /* 写入本次轮询延时使用的计数初值。 */
    REG_TIMER_0_LOAD_COUNT = ticks;

    /* 选择用户定义周期并保持完成状态非屏蔽。 */
    REG_TIMER_0_CONTROL = STARTYSKY_T1_PICO_TIMER_CONTROL_USER;
    __asm__ volatile("fence iorw, iorw" : : : "memory");

    /* 设置使能位并开始本次计数。 */
    REG_TIMER_0_CONTROL = STARTYSKY_T1_PICO_TIMER_CONTROL_USER |
                          STARTYSKY_T1_PICO_TIMER_CONTROL_ENABLE;
    __asm__ volatile("fence iorw, iorw" : : : "memory");
 ...
 */
static uint8_t startysky_t1_pico_timer_delay_scaled(uint8_t timer_id,
                                               uint32_t value,
                                               uint32_t ticks_per_unit)
{
    uint32_t maximum_units;

    /* StartySky T1-Pico 当前只提供 Timer0 给通用 Timer HAL 使用。 */
    if ((timer_id != 0u) || (ticks_per_unit == 0u) ||
        (startysky_t1_pico_timer_interrupt_active != 0u))
        return 1u;
    ...
    while (value != 0u)
    {
        ...
        /* 等待当前分块对应的硬件时钟数。 */
        if (startysky_t1_pico_timer_wait_ticks(current_units * ticks_per_unit) != 0u)
            return 1u;
...
uint8_t hal_delay_us(uint8_t timer_id, uint32_t val)
{
    ...
    return startysky_t1_pico_timer_delay_scaled(timer_id,
                                           val,
                                           STARTYSKY_T1_PICO_TIMER_CLOCK_HZ / 1000000u);
}
...
uint8_t hal_delay_ms(uint8_t timer_id, uint32_t val)
{
    ...
    return startysky_t1_pico_timer_delay_scaled(timer_id,
                                           val,
                                           STARTYSKY_T1_PICO_TIMER_CLOCK_HZ / 1000u);
}
...
uint8_t hal_delay_s(uint8_t timer_id, uint32_t val)
{
    ...
    return startysky_t1_pico_timer_delay_scaled(timer_id,
                                           val,
                                           STARTYSKY_T1_PICO_TIMER_CLOCK_HZ);
}
...
{
    /* StartySky T1-Pico 当前只接受 Timer0 对应的系统滴答编号。 */
    if (timer_id != 0u)
        return 1u;
...
uint32_t hal_get_sys_tick(uint8_t timer_id)
{
    ...
    /* 将五兆赫兹 CLINT 计数转换为毫秒。 */
    return elapsed_ticks / (STARTYSKY_T1_PICO_CLINT_MTIME_FREQ_HZ / 1000u);
}
...
int hal_timer_init(uint8_t timer_id, const hal_timer_config_t *config)
{
    ...
    REG_TIMER_0_CONTROL = STARTYSKY_T1_PICO_TIMER_CONTROL_USER |
                          STARTYSKY_T1_PICO_TIMER_CONTROL_MASK;
    __asm__ volatile("fence iorw, iorw" : : : "memory");
    return 0;
}
...
int hal_timer_start(uint8_t timer_id)
{
    ...
    /* 保留周期和中断屏蔽配置并设置使能位。 */
    REG_TIMER_0_CONTROL |= STARTYSKY_T1_PICO_TIMER_CONTROL_ENABLE;
    __asm__ volatile("fence iorw, iorw" : : : "memory");
    return 0;
}
