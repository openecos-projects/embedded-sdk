...
extern uint8_t starrysky_t1_pico_timer_interrupt_active;

/**
 * 清除 Timer0 中断并调用用户处理函数。
 */
static void starrysky_t1_pico_timer0_intr_dispatch(void *arg)
{
                                ...
                                uint32_t priority)
{
    ...
    if (hal_intr_alloc(STARRYSKY_T1_PICO_TIMER0_PLIC_SOURCE,
                       priority,
                       starrysky_t1_pico_timer0_intr_dispatch,
                       NULL) != 0)
    {
    ...
    REG_TIMER_0_CONTROL &= ~STARRYSKY_T1_PICO_TIMER_CONTROL_MASK;
    if (hal_intr_enable(STARRYSKY_T1_PICO_TIMER0_PLIC_SOURCE) != 0)
    {
        REG_TIMER_0_CONTROL |= STARRYSKY_T1_PICO_TIMER_CONTROL_MASK;
        (void)hal_intr_free(STARRYSKY_T1_PICO_TIMER0_PLIC_SOURCE);
        timer0_callback = NULL;
        timer0_callback_arg = NULL;
        return -1;
    }
    ...
    starrysky_t1_pico_timer_interrupt_active = 1u;
    __asm__ volatile("fence iorw, iorw" : : : "memory");
    return 0;
}
...
int hal_timer_unregister_callback(uint8_t timer_id)
{
    ...
    REG_TIMER_0_CONTROL |= STARRYSKY_T1_PICO_TIMER_CONTROL_MASK;
    (void)hal_intr_disable(STARRYSKY_T1_PICO_TIMER0_PLIC_SOURCE);
    (void)hal_timer_clear_interrupt(0u);

    /* 释放 PLIC source 并清除用户处理信息。 */
    if (hal_intr_free(STARRYSKY_T1_PICO_TIMER0_PLIC_SOURCE) != 0)
        return -1;
