...
void hal_sys_uart_init(void)
{
    ...
    divisor = (STARRYSKY_T1_PICO_APB_CLOCK_HZ + (baud_rate * 8u)) /
              (baud_rate * 16u);
    if (divisor == 0u)
        divisor = 1u;

    /* 将过大的分频值限制到硬件十六位寄存器范围内。 */
    if (divisor > STARRYSKY_T1_PICO_UART_DIVISOR_MAX)
        divisor = STARRYSKY_T1_PICO_UART_DIVISOR_MAX;

    /* 使能分频锁存器并写入十六位分频值。 */
    REG_UART_0_LCR = STARRYSKY_T1_PICO_UART_LCR_DLAB;
...
{
    /* 等待发送保持寄存器能够接收新字符。 */
    while ((REG_UART_0_LSR & STARRYSKY_T1_PICO_UART_LSR_TX_READY) == 0u)
    {
    }
...
{
    /* 等待接收保持寄存器中出现有效数据。 */
    while ((REG_UART_0_LSR & STARRYSKY_T1_PICO_UART_LSR_DATA_READY) == 0u)
    {
    }
