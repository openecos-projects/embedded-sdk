...
static gpio_hal_isr_t gpio_handlers[STARRYSKY_T1_PICO_GPIO_PINS_PER_PORT];
static void *gpio_handler_args[STARRYSKY_T1_PICO_GPIO_PINS_PER_PORT];

/**
 * 判断 GPIO 中断端口和引脚是否由当前硬件支持。
 */
static uint8_t starrysky_t1_pico_gpio_intr_is_valid(uint8_t gpio_id,
                                               uint8_t gpio_num)
{
    /* 当前仅 GPIO port A 的八个引脚连接到独立 PLIC source。 */
    return (uint8_t)((gpio_id == STARRYSKY_T1_PICO_GPIO_PORT_A) &&
                     (gpio_num < STARRYSKY_T1_PICO_GPIO_PINS_PER_PORT));
}
 ...
 */
static uint32_t starrysky_t1_pico_gpio_intr_source(uint8_t gpio_num)
{
    /* GPIOA0 至 GPIOA7 依次映射到 PLIC source 8 至 15。 */
    return STARRYSKY_T1_PICO_GPIOA_PLIC_SOURCE_BASE + gpio_num;
}
 ...
 */
static void starrysky_t1_pico_gpio_intr_dispatch(void *arg)
{
                           ...
                           gpio_intr_type_t intr_type)
{
    ...
    if ((starrysky_t1_pico_gpio_intr_is_valid(gpio_id, gpio_num) == 0u) ||
                             ...
                             uint32_t priority)
{
    ...
    source = starrysky_t1_pico_gpio_intr_source(gpio_num);
    if (hal_intr_alloc(source,
                       priority,
                       starrysky_t1_pico_gpio_intr_dispatch,
