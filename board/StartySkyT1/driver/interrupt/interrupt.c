...
#define STARTYSKY_T1_MSTATUS_MIE             (1u << 3)
#define STARTYSKY_T1_MIE_MEIE                (1u << 11)
#define STARTYSKY_T1_MCAUSE_MACHINE_EXTERNAL 0x8000000Bu

struct startysky_t1_intr_entry
{
    hal_intr_handler_t handler;
    void *arg;
};

extern void startysky_t1_machine_trap_entry(void);

static struct startysky_t1_intr_entry
    interrupt_entries[STARTYSKY_T1_PLIC_SOURCE_COUNT];
static uint8_t interrupt_initialized;

/**
 * 保存机器全局中断状态并临时关闭全局中断。
 */
static uint32_t startysky_t1_intr_lock(void)
{
    uint32_t previous;
 ...
 * 根据保存的机器状态恢复全局中断使能位。
 */
static void startysky_t1_intr_unlock(uint32_t previous)
{
    /* 仅在进入临界区前已经开启时恢复机器全局中断。 */
    if ((previous & STARTYSKY_T1_MSTATUS_MIE) != 0u)
 ...
 */
static uint8_t startysky_t1_intr_source_is_valid(uint32_t source)
{
    /* source 0 保留为无中断，当前 enable 寄存器覆盖 source 1 至 31。 */
    return (uint8_t)((source != 0u) &&
                     (source < STARTYSKY_T1_PLIC_SOURCE_COUNT));
}
...
int hal_intr_init(void)
{
    ...
    previous = startysky_t1_intr_lock();
    REG_PLIC_ENABLE = 0u;
    ...
    /* 安装四字节对齐的机器模式直接陷阱入口。 */
    entry = (uintptr_t)startysky_t1_machine_trap_entry;
    ...
    startysky_t1_intr_unlock(previous);
    return 0;
}
                   ...
                   void *arg)
{
    ...
    if ((startysky_t1_intr_source_is_valid(source) == 0u) ||
 ...
 */
void startysky_t1_machine_trap_handler(uint32_t mcause, uint32_t mepc)
{
    hal_intr_handler_t handler;
    void *arg;
    uint32_t source;
    ...
    /* 非机器外部中断交给可覆盖的异常处理函数。 */
    if (mcause != STARTYSKY_T1_MCAUSE_MACHINE_EXTERNAL)
