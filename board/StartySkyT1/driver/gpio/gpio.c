...
static volatile uint32_t *const gpio_data_registers[STARTYSKY_T1_GPIO_PORT_COUNT] =
{
...
static volatile uint32_t *const gpio_direction_registers[STARTYSKY_T1_GPIO_PORT_COUNT] =
{
...
static volatile uint32_t *const gpio_input_registers[STARTYSKY_T1_GPIO_PORT_COUNT] =
{
...
/**
 * 判断 GPIO 端口和引脚编号是否位于 StartySky T1 有效范围内。
 */
static uint8_t startysky_t1_gpio_is_valid(uint8_t gpio_id, uint8_t gpio_num)
{
    /* 同时检查四组端口和每组低八位引脚范围。 */
    return (uint8_t)((gpio_id < STARTYSKY_T1_GPIO_PORT_COUNT) &&
...
void gpio_hal_input_enable(uint8_t gpio_id, uint8_t gpio_num)
{
    ...
    /* 忽略超出 StartySky T1 GPIO 范围的配置请求。 */
    if (startysky_t1_gpio_is_valid(gpio_id, gpio_num) == 0u)
        return;
...
void gpio_hal_set_level(uint8_t gpio_id, uint8_t gpio_num, uint8_t level)
{
    ...
    /* 忽略超出 StartySky T1 GPIO 范围的输出请求。 */
    if (startysky_t1_gpio_is_valid(gpio_id, gpio_num) == 0u)
        return;
