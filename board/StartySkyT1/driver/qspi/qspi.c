...
#define STARTYSKY_T1_QSPI_DFS_8_BIT             (7u << 16)
#define STARTYSKY_T1_QSPI_DFS_16_BIT            (15u << 16)
#define STARTYSKY_T1_QSPI_DFS_32_BIT            (31u << 16)
#define STARTYSKY_T1_QSPI_TMOD_TX_ONLY          (1u << 8)
#define STARTYSKY_T1_QSPI_TMOD_EEPROM_READ      (3u << 8)
#define STARTYSKY_T1_QSPI_STATUS_BUSY           (1u << 0)
#define STARTYSKY_T1_QSPI_STATUS_TX_NOT_FULL    (1u << 1)
#define STARTYSKY_T1_QSPI_STATUS_TX_EMPTY       (1u << 2)
#define STARTYSKY_T1_QSPI_STATUS_RX_NOT_EMPTY   (1u << 3)
#define STARTYSKY_T1_QSPI_RX_OVERFLOW           (1u << 3)
#define STARTYSKY_T1_QSPI_TIMEOUT               1000000u
 ...
 */
static uint32_t startysky_t1_qspi_chip_select(hal_qspi_cs_t cs)
{
 ...
 */
static void startysky_t1_qspi_reset(void)
{
    /* 禁用控制器并取消全部片选。 */
 ...
 * 配置 QSPI 数据帧和传输模式。
 */
static void startysky_t1_qspi_configure(uint32_t ctrlr0, uint32_t receive_frames)
{
    /* 修改传输格式前复位控制器和 FIFO。 */
    startysky_t1_qspi_reset();
 ...
 */
static int startysky_t1_qspi_wait_tx_space(void)
{
    uint32_t timeout = STARTYSKY_T1_QSPI_TIMEOUT;

    /* 轮询发送 FIFO 非满状态。 */
    while (((REG_QSPI_0_SR & STARTYSKY_T1_QSPI_STATUS_TX_NOT_FULL) == 0u) &&
           (timeout != 0u))
        --timeout;
 ...
 */
static int startysky_t1_qspi_wait_idle(void)
{
    uint32_t timeout = STARTYSKY_T1_QSPI_TIMEOUT;
    ...
    while (timeout != 0u)
    {
        ...
        if (((status & STARTYSKY_T1_QSPI_STATUS_BUSY) == 0u) &&
            ((status & STARTYSKY_T1_QSPI_STATUS_TX_EMPTY) != 0u))
            return 0;
 ...
 */
static int startysky_t1_qspi_wait_rx_data(void)
{
    uint32_t timeout = STARTYSKY_T1_QSPI_TIMEOUT;
    ...
    while (timeout != 0u)
    {
        if ((REG_QSPI_0_RISR & STARTYSKY_T1_QSPI_RX_OVERFLOW) != 0u)
            return -1;

        if ((REG_QSPI_0_SR & STARTYSKY_T1_QSPI_STATUS_RX_NOT_EMPTY) != 0u)
            return 0;
 ...
 * 使用指定片选发送一组同宽数据帧。
 */
static int startysky_t1_qspi_write_frames(const uint32_t *frames,
                                  ...
                                  hal_qspi_cs_t cs)
{
    ...
    /* 检查数据缓冲区、帧数量和片选编号。 */
    chip_select = startysky_t1_qspi_chip_select(cs);
    if ((frames == NULL) || (frame_count == 0u) || (chip_select == 0u))
        return -1;

    /* 配置仅发送模式并在片选无效时预填第一帧。 */
    startysky_t1_qspi_configure(frame_format | STARTYSKY_T1_QSPI_TMOD_TX_ONLY, 0u);
    ...
    {
        if (startysky_t1_qspi_wait_tx_space() != 0)
        {
            REG_QSPI_0_SER = 0u;
            return -1;
        }
    ...
    /* 等待全部串行传输完成并释放片选。 */
    if (startysky_t1_qspi_wait_idle() != 0)
    {
        REG_QSPI_0_SER = 0u;
        return -1;
    }
 ...
 */
static int startysky_t1_qspi_write_header(uint8_t cmd,
                                  uint8_t cmd_len,
                                  uint32_t addr,
                                  uint8_t addr_len)
{
 ...
 * 检查新 QSPI API 的公共参数。
 */
static int startysky_t1_qspi_validate_transaction(hal_qspi_port_t port,
                                          uint8_t cmd_len,
                                          uint8_t addr_len)
{
    /* 检查端口、命令宽度和字节对齐的地址宽度。 */
...
int hal_qspi_init(hal_qspi_port_t port, const hal_qspi_config_t *config)
{
    ...
    startysky_t1_qspi_configure(STARTYSKY_T1_QSPI_DFS_8_BIT | STARTYSKY_T1_QSPI_TMOD_TX_ONLY, 0u);
    return 0;
}


/**
 * 关闭 StartySky T1 QSPI0 控制器。
 */
int hal_qspi_deinit(hal_qspi_port_t port)
{
    /* 拒绝 StartySky T1 不支持的 QSPI 端口编号。 */
                             ...
                             uint32_t words)
{
    ...
    /* 配置三十二位仅发送模式并预填第一帧。 */
    startysky_t1_qspi_configure(STARTYSKY_T1_QSPI_DFS_32_BIT | STARTYSKY_T1_QSPI_TMOD_TX_ONLY, 0u);
    REG_QSPI_0_DR = data;
    REG_QSPI_0_SER = startysky_t1_qspi_chip_select(HAL_QSPI_CS_0);
                        ...
                        hal_qspi_cs_t cs)
{
    ...
    return startysky_t1_qspi_write_frames(&frame, 1u, STARTYSKY_T1_QSPI_DFS_8_BIT, cs);
}
                         ...
                         hal_qspi_cs_t cs)
{
    ...
    return startysky_t1_qspi_write_frames(&frame, 1u, STARTYSKY_T1_QSPI_DFS_16_BIT, cs);
}
                         ...
                         hal_qspi_cs_t cs)
{
    ...
    return startysky_t1_qspi_write_frames(&data, 1u, STARTYSKY_T1_QSPI_DFS_32_BIT, cs);
}
                           ...
                           hal_qspi_cs_t cs)
{
    ...
    return startysky_t1_qspi_write_frames(frames, 2u, STARTYSKY_T1_QSPI_DFS_32_BIT, cs);
}
                           ...
                           hal_qspi_cs_t cs)
{
    ...
    return startysky_t1_qspi_write_frames(frames, 8u, STARTYSKY_T1_QSPI_DFS_32_BIT, cs);
}
                            ...
                            hal_qspi_cs_t cs)
{
    ...
    return startysky_t1_qspi_write_frames(frames, 16u, STARTYSKY_T1_QSPI_DFS_32_BIT, cs);
}
                            ...
                            hal_qspi_cs_t cs)
{
    ...
    return startysky_t1_qspi_write_frames(frames, 32u, STARTYSKY_T1_QSPI_DFS_32_BIT, cs);
}
...
{
    /* 检查标准 SPI 命令和地址参数。 */
    if (startysky_t1_qspi_validate_transaction(port, cmd_len, addr_len) != 0)
        return -1;

    /* 配置八位仅发送模式并写入命令和地址。 */
    startysky_t1_qspi_configure(STARTYSKY_T1_QSPI_DFS_8_BIT | STARTYSKY_T1_QSPI_TMOD_TX_ONLY, 0u);
    if (startysky_t1_qspi_write_header(cmd, cmd_len, addr, addr_len) != 0)
        return -1;
                  ...
                  uint16_t rx_len)
{
    ...
    startysky_t1_qspi_configure(STARTYSKY_T1_QSPI_DFS_8_BIT | STARTYSKY_T1_QSPI_TMOD_EEPROM_READ,
                       (uint32_t)rx_len - 1u);
    if (startysky_t1_qspi_write_header(cmd, cmd_len, addr, addr_len) != 0)
        return -1;
    ...
    {
        if (startysky_t1_qspi_wait_rx_data() != 0)
        {
            REG_QSPI_0_SER = 0u;
            return -1;
        }
