...
void rtc_test(){
  ECOS_LOGI("rtc", "[TEST_START] rtc_test");
  ...
  for(uint32_t i = 0; i < 3; ++i) {
    ...
    ECOS_LOGI("rtc", "[static]CNT: %d", hal_rtc_get_cnt());
  }
  hal_rtc_set_cnt(0);
  hal_rtc_set_ctrl(0b0010010); // core and inc trg en
  
  ECOS_LOGI("rtc", "cnt inc test");
  for(int i = 0; i < 3; ++i) {
    while(hal_rtc_get_ista() != 1); 
    ECOS_LOGI("rtc", "RTC_REG_CNT: %d", hal_rtc_get_cnt());
  }
  ECOS_LOGI("rtc", "cnt inc test done");
