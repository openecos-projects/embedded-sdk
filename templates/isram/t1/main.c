...
{
    uintptr_t address;
    uintptr_t end = STARTYSKY_T1_ISRAM_BASE_ADDR +
                    STARTYSKY_T1_ISRAM_TEST_SIZE_BYTES;

    /* 逐字写入整个十六千字节测试范围。 */
    for (address = STARTYSKY_T1_ISRAM_BASE_ADDR;
         address < end;
...
int main(void)
{
    /* 初始化串口并测试两组互补地址模式。 */
    hal_sys_uart_init();
    hal_sys_putstr("StartySky T1 ISRAM 16 KiB test started.\n");
